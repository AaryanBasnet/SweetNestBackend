...
const initiatePayment = asyncHandler(async (req, res) => {
  ...
  // Append orderId so we can find the order if eSewa returns no data
  const failureUrl = `${
    process.env.BACKEND_URL || "http://localhost:5050"
  }/api/esewa/failed?orderId=${order._id}`;
...
const verifyPayment = asyncHandler(async (req, res) => {
  ...
  if (!data) {
    return res.redirect(
      `${FRONTEND_URL}/checkout?status=error&message=Invalid%20payment%20response`
    );
  }
  ...
  try {
    ...
    if (signature !== expectedSignature) {
      ...
      return res.redirect(
        `${FRONTEND_URL}/checkout?status=error&message=Payment%20verification%20failed`
      );
    }

    // Find order by transaction UUID
    const order = await Order.findOne({
      "esewa.transactionId": transaction_uuid,
    });

    if (!order) {
      console.error("Order not found for transaction:", transaction_uuid);
      return res.redirect(
        `${FRONTEND_URL}/checkout?status=error&message=Order%20not%20found`
      );
    }
    ...
    if (parseFloat(total_amount) !== order.total) {
      ...
      return res.redirect(
        `${FRONTEND_URL}/checkout?status=error&message=Amount%20mismatch`
      );
    }
    ...
    if (status === "COMPLETE") {
      ...
      return res.redirect(
        `${FRONTEND_URL}/checkout?status=success&orderId=${order._id}&orderNumber=${order.orderNumber}`
      );
    } else {
      ...
      await order.save();
      return res.redirect(
        `${FRONTEND_URL}/checkout?status=failed&message=Payment%20was%20not%20completed`
      );
    }
  } catch (error) {
    console.error("eSewa verification error:", error);
    return res.redirect(
      `${FRONTEND_URL}/checkout?status=error&message=Payment%20verification%20error`
    );
  }
});
...
const handleFailure = asyncHandler(async (req, res) => {
  const { orderId } = req.query; // FIXED: Get the ID from the URL we set above
  ...
  res.redirect(
    `${FRONTEND_URL}/checkout?status=failed&message=Payment%20was%20cancelled%20or%20failed`
  );
});
